...
The result? The VM caught the automated probe, calculated that $5 \ge 3$ failed attempts within the 60-second window, and instantly fired an alert straight to my phone. Mission accomplished.⚙️ Tech Stack Stacked DeepCore Language: Bash / Linux Shell ScriptingLog Aggregation: systemd-journald & journalctlNetwork Tuning: Linux Kernel Namespace (sysctl)API Gateways: Telegram Bot API via curl HTTPS POSTHypervisor: Microsoft Hyper-V
